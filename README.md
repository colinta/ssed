...
15 minutes later I had `fed`. I think the README took longer than the tool!
...
Replaces the first match.
    ...
    echo what a test | fed sub/w/W
    => What a test
    echo what a test | fed 's/(\w+)/$1$1'
    => whatwhat a test
...
Replaces all occurences of the pattern.
    ...
    echo what a test | fed gsub/t/T
    => whaT a TesT
    echo what a test | fed g/[a-su-z]/.
    => ...t . t..t
...
Prints the matching regex (or the entire line if there is no match).
    ...
    echo 'what a test' | fed 'take/t\w+'
    => test
    echo -e "what
    > a
    > great test" | fed '+/t\w+t'
    => what
    a
    test
...
Prints the n-th group in the regex (or the entire line if there is no match).
    ...
    echo 'what a test' | fed '1/(\w+) (\w+) (\w+)'
    => what
    echo 'what a test' | fed '2/(\w+) (\w+) (\w+)'
    => a
    echo 'what a test' | fed '3/(\w+) (\w+) (\w+)'
    => test
...
Removes the matching part of the regex (or the entire line if there is no match).
    ...
    echo 'what a test' | fed 'rm/t\w+'
    => what a
    echo -e "what
    > a
    > great test" | fed 'rm/t\w+t'
    => what
    a
    great
